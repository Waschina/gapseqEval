predGeneEssentiality.CM <- function(mod, loci.file, gess.file, cm.loci.file) {
  multi.contig.mode <- F
  ...
  model.genes <- fread(cm.loci.file, header = F)
  colnames(model.genes) <- c("mod.gene","contig","start","end")
  
  
  model.genes$mod.gene <- paste0("G_",model.genes$mod.gene)
